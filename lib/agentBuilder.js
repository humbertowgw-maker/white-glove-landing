...
export async function fetchDailyReports(token, limit = 7) {
  const res = await fetch(`${API}/api/agents-registry/daily-reports?limit=${limit}`, {
    headers: headers(token),
  });
  return parseResponse(res);
}
...
export async function fetchLanguageProfiles(token, { limit = 100, leadId } = {}) {
  const url = new URL(`${API}/api/agents-registry/language-profiles`);
